...
| `npm run make` / `package` hangs at “Preparing native dependencies”, no `out/` folder | Run `npm run package:debug` and read the error. Common: `package.json` `main` must be `.webpack/main` (not `.webpack/main/index.js`). Also ensure `rebuildConfig.onlyModules: []` in `forge.config.ts`. |
